...
function Resolve-PgBin {
    ...
    $toolDirClean = Join-Path $PSScriptRoot "tools\pgsql18clean\pgsql\bin"
    if (Test-Path "$toolDirClean\pg_dump.exe") { return (Resolve-Path $toolDirClean).Path }
...
if (-not $SkipDump) {
    Write-Host ('=== Step 1: Dump ServBay ({0}:{1}/{2}) ===' -f $ServBayHost, $ServBayPort, $ServBayDb)
    ...
    Write-Host ('Dump saved: {0} ({1} bytes)' -f $DumpFile, (Get-Item $DumpFile).Length)
} else {
    if (-not (Test-Path $DumpFile)) { throw "Dump file not found: $DumpFile" }
    Write-Host "SkipDump set - using existing dump: $DumpFile"
}
...
if ($RailwayHost -and $RailwayPort -gt 0) {
    $railway = @{
        Host = $RailwayHost
        Port = $RailwayPort
        Database = $RailwayDb
