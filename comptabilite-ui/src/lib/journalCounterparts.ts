/**
 * OHADA double-entry — counterpart filtering and side locking (ported from WYVERN Core_Account).
 */
...
export function ohadaClassFromCode(code: string): number {
  const c = String(code || '').trim();
  if (!c || !/^\d/.test(c)) return 0;
  return parseInt(c[0], 10);
}

export function lineSide(line: JournalLineLike | undefined): 'debit' | 'credit' | null {
...
export function mapJournalTypeToCode(journalType: string): string {
  switch (journalType) {
    case 'TJE':
      return 'OD';
    default:
...
export function naturalSideForAccount(account: JournalAccountLike | undefined, journalCode = 'OD'): 'debit' | 'credit' | null {
...
export function expectedSideForLine(
  lineIndex: number,
  allLines: JournalLineLike[],
  journalCode: string,
  accountsByCode: Map<string, JournalAccountLike>
): 'debit' | 'credit' | null {
...
export function postingSideForLine(
  lineIndex: number,
  line: JournalLineLike,
...
export function sideFieldState(postingSide: 'debit' | 'credit' | null) {
...
export function sideHintForLine(
  lineIndex: number,
  line: JournalLineLike,
  _account: JournalAccountLike | undefined,
  journalCode: string,
  allLines: JournalLineLike[],
  accountsByCode: Map<string, JournalAccountLike>
): string {
...
function paymentMethodCounterparts(pool: JournalAccountLike[], journalCode: string) {
...
function sortByPriority(accounts: JournalAccountLike[], priorityCodes: string[], journalCode: string) {
  const journalFirst =
    journalCode === 'CA'
...
function isTreasury(code: string) {
  return ohadaClassFromCode(code) === 5;
}

function isReceivable(code: string) {
...
function isRevenue(account: JournalAccountLike) {
  const cls = account.ohada_class ?? ohadaClassFromCode(account.code);
  return cls === 7 || account.account_type === 'income' || account.account_type === 'revenue';
}

export function linePairsWithRevenueCredit(
  lineIndex: number,
  allLines: JournalLineLike[],
  accountsByCode: Map<string, JournalAccountLike>,
  journalCode: string
...
function isExpense(account: JournalAccountLike) {
  const cls = account.ohada_class ?? ohadaClassFromCode(account.code);
  return cls === 6 || account.account_type === 'expense' || account.account_type === 'cost';
}

function isHospitalRevenueCode(code: string) {
  const c = String(code);
  return (
    c.startsWith('7016') ||
    c.startsWith('7026') ||
...
export function filterCounterpartAccounts(
  sourceAccount: JournalAccountLike,
  sourceSide: 'debit' | 'credit',
  allAccounts: JournalAccountLike[],
  journalCode = 'OD'
): JournalAccountLike[] {
...
export function pickDefaultCounterpart(counterparts: JournalAccountLike[], journalCode: string) {
...
export function counterpartHint(sourceAccount: JournalAccountLike, sourceSide: 'debit' | 'credit'): string {
...
export function lineTotals(lines: JournalLineLike[]) {
  let debit = 0;
  let credit = 0;
...
export function applySideLockToLineValues(
  line: JournalLineLike,
  postingSide: 'debit' | 'credit' | null
): Pick<JournalLineLike, 'debitAmount' | 'creditAmount'> {
